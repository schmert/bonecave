...
## legislative info -- have to manually remove Loeffler (GA) and add
## Senate runoff winners Warnock (GA) and Ossof (GA) 
## for pyramid (and political) purposes, count Independents as Democrats
## also add a fake "district" 1 or 2, so that states with
## mixed party representation in Senate can be split

legis = read_csv('legislators-current.csv') %>% 
     filter(type=='sen', state %in% state.abb, state != 'GA') %>% 
     select(full_name, gender, state, party) %>% 
     mutate( party = ifelse(party=='Republican','Republican','Democrat')) %>% 
     arrange(state) %>% 
     transform( dist_num = 1:2)

extra = tribble(
  ~full_name, ~gender, ~state, ~party, ~dist_num,
  'Raphael Warnock', 'M', 'GA', 'Democrat',1,
  'Jon Ossof', 'M', 'GA', 'Democrat',2
)

legis = rbind( legis, extra)

head(legis)


## census info by state
# get API key
...
tmp = get_acs(geography = 'state',
            ...
            year=2019,
            output = 'tidy') %>% 
      left_join(fips, by=c('GEOID'='state_code')) %>% 
      left_join(Table1_info)

# quick and dirty trick -- divide each population into two 'districts', each with
# 1/2 of population

tmp = tmp %>% 
       arrange(state, sex, L) %>% 
       mutate( estimate = .50*estimate)
...
D1 = left_join(legis, P1, by=c('state','dist_num')) %>% 
      select(-NAME,-moe,-state_num) %>% 
      group_by(party,sex,race,L,H) %>% 
...
G = G + 
  ...
  labs(title='US Senate Representation by Age, Sex, and Race/Ethnicity',
...
ggsave(plot=G, filename='population-pyramids-by-party-senate.png',
