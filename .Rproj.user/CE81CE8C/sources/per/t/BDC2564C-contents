...
BTR = readHFD('AUTbirthsTR.txt')  %>% 
  rename(Births=Total)

NTR = readHFD('AUTexposTR.txt')
...
BRR = readHFD('AUTbirthsRR.txt') %>% 
  rename(Births=Total)

NRR = readHFD('AUTexposRR.txt')
...
G = ggplot(data=df) + 
  ...
  labs(title='AUT Cohort Fertility over ages 13-40',
       y='CFR40',
       subtitle='Line = Avg of consecutive A-P squares')

print(G)

ggsave(filename='AUT-CFR-levels.png',
...
ggsave(filename='AUT-CFR-errors.png',       
       height=8, width=8, units='in')
