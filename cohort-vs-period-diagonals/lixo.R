...
for (i in 1:nrow(cases)) {
 ...
 # calculate K multipliers for approximate 
 # (size of next cohort / size of current cohort)
 # from the AP squares
 # This particular method uses the cumulative
 # age-period exposure over ages 20-39
  ...
  Kwts = NRR %>% 
          ...
          group_by(Cohort) %>% 
          summarize(N = sum(N[Age %in% 20:39])) %>% 
          mutate(nextN = lead(N,1),
                 K = nextN/N) %>% 
          filter(Cohort %in% TFR_triangle$Cohort) %>% 
          select(Cohort,K)
  ...
  TFR_realnew = TFR_rectangle %>%
    ungroup() %>% 
    left_join(Kwts) %>% 
    mutate(TFR = (TFR + K*lead(TFR,1))/(1+K),
           method='AP2wtd')
  ...
  G = ggplot(data=df) + 
    ...
    theme_bw() +
    scale_shape_manual(values=c('square','cross','diamond','triangle')) +
  ...
  err_df = df %>% 
    ...
    summarize(e_AP  = 100*(TFR[method=='AP']/TFR[method=='True CFR']-1),
              ...
              e_AP2wtd = 100*(TFR[method=='AP2wtd']/TFR[method=='True CFR']-1)) %>% 
