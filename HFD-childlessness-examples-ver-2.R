...
for (this_code in c('USA','JPN','CZE')) {
  ...
  tmp = readHFDweb(CNTRY=this_code, item='tfrVHbo', 
         ...
         select(country,cohort=Cohort,CCF40_1,CCF1) %>% 
         mutate(childless = 100*(1-CCF40_1),
                childless50 = 100*(1-CCF1))
...
G = ggplot(data=df) +
  ...
  geom_smooth(aes(y=childless50),span=.40,se=FALSE, 
              lwd=0.5) +
  theme_bw() +
  labs(x='Year of Birth\n(Current Age)',y='% Childless at Age 40',
       title="Percent of Women Childless at Age 40 (upper line) & 55 (lower)",
...
print(G)
  ...
  ggsave(filename=here('HFD-childless-examples-ver-2.png'), plot=G,
